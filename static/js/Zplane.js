...
class Zplane {
    ...
    //delete allpass 
    delete_allpass = () => {
        //delete allpass from dataSet
        for (let i = 0; i < this.dataSet.length; i++) {
            if ((this.dataSet[i].type === this.types.allPass) || (this.dataSet[i].type === this.types.allPass)) {
                this.dataSet.splice(i, 1);
                i--;
            }
        }
        // get all points with type nonConjZero and conjZero in dom and delete it 
        document.querySelectorAll(".point").forEach(node => {
            if ((node.getAttribute("type") === this.types.allPass) ||
                (node.getAttribute("type") === this.types.allPass)) {
                node.remove();
            }
        });
    }
            ...
            points.enter().append("text").text("X").attr("x", (d) => {
                ...
                .call(d3.drag()
                    .on("drag", (ev) => {
                        ...
                        // if (Math.sqrt(Math.pow(Number(ev.x), 2) + Math.pow(Number(ev.y), 2)) > 360){
                        //     xElement = 0;
                        //     yElement = 0;
                        // }
                        // else{
